...
func NewPod(config *PodConfig, ps *util.PubSub, mgr manager.Manager) *PodCtrl {
	...
	err = ctrl.NewControllerManagedBy(mgr).
		...
		WithEventFilter(predicate.NewPredicateFuncs(func(object ctrclient.Object) bool {
			// namespace filter
			if newctl.config.Namespace != "" && object.GetNamespace() != config.Namespace {
				return false
			}
			// labels filter
		...
		Complete(newctl)
	if err != nil {
		klog.Fatalf("create pod controller failed: %v", err)
	}
	return newctl
}
...
func (h *PodCtrl) syncOnce(ctx context.Context) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("init sync failed, list pod error: %w", err)
	}
...
func (h *PodCtrl) ListPodHostIP(_ context.Context, publishNotready bool) ([]string, error) {
	var (
		hostips []string
	)
	select {
